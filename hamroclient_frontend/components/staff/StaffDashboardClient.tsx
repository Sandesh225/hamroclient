...
export default function StaffDashboardClient() {
  const { data, isLoading, isFetching } = useGetStaffDashboardQuery(undefined, {
    pollingInterval: 30000,
  });
  
  const metrics = data?.metrics || {
    applicationsRegisteredThisWeek: 0,
    missingDocuments: 0,
    pendingDecisions: 0,
    activeApplicants: 0,
  };
            ...
            {urgentTasks.length === 0 ? (
              <div className="p-8 flex flex-col items-center text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1 max-w-xs">
                  No urgent tasks right now. New items will appear here when applicant cases need your attention.
                </p>
              </div>
            ) : (
                  ...
                  <tr>
                    <td colSpan={5} className="py-8">
                      <div className="flex flex-col items-center text-center">
                        ...
                        <p className="text-sm font-medium text-foreground">No Recent Applicants</p>
                        <p className="text-xs text-muted-foreground mt-1">
                          Applicants assigned to you will appear here once created.
                        </p>
                      </div>
                    </td>
                  </tr>
